...
function NFTCard({nft,execute}) {
  ...
  <button type='button' onClick={()=>execute(nft)} className="bid_btn pc_bid">{(nft.Status=='Fixedprice'||nft.Status=='Auction')?"On Sale":"View"}
    </button> 
  {/* <button type='button' onClick={()=>execute(nft)} className="bid_btn mobile_bid"> Bid
    </button>  */}
    <button type='button' onClick={()=>execute(nft)} className="bid_btn mobile_bid">{(nft.Status=='Fixedprice'||nft.Status=='Auction')?"On Sale":"View"}
    </button> 
